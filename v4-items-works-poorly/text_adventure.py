...
def get_item_at_location(location):
    commands = location['options'].keys()
...
def show_description(location, visited):
    ...
    item = get_item_at_location(location)
    player_has_item = 'item' in visited

    if first_time_in_location or no_second_description or item == None or not player_has_item:
